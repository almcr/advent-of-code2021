...
fn recurse(pos: i32, bites: Vec<i32>, cmp: fn(&usize, &usize) -> bool) -> i32
  ...
  let most = cmp(&bites.iter().filter(|x| is_set(x, pos)).count(), &((bites.len() + 1) / 2));
  recurse(pos - 1, bites.into_iter().filter(|x| is_set(x, pos) == most).collect(), cmp)
}

fn day3a() -> Result<i32, Box<dyn std::error::Error>> {
  let input = include_str!("./input");
  let bites = input.lines().map(|s| i32::from_str_radix(s, 2).unwrap()).collect::<Vec<_>>();
